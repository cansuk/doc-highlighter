...
/* --- panels -----------------------------------------------------------------
 * Five unrelated things share this document. They used to run as one long
 * sequence of top-level awaits, so a throw anywhere blanked everything after it
 * — silently, because nothing reported. That is how a working Recent list could
 * end up looking like an empty one.
 *
 * Each panel is set up on its own now. One failing leaves the others alone and
 * says what happened in place, rather than leaving a blank for somebody to
 * misread as "there is nothing here".
 * ------------------------------------------------------------------------- */

async function panel(name, nodeId, fn) {
  try {
    await fn();
  } catch (e) {
    console.error(`[DocHL popup] ${name}:`, e);
    const box = nodeId && document.getElementById(nodeId);
    if (!box) return;
    const p = document.createElement('p');
    p.className = 'none';
    p.textContent = `${t('qmPanelFailed')} — ${e?.message ?? e}`;
    box.textContent = '';
    box.appendChild(p);
  }
}
...
/** The same vocabulary the page uses, so the list and the document agree. */
function swatchFor(h, custom) {
  const colour = (name) => custom[name] || DEFAULTS[name];
  const sw = document.createElement('span');
  if (h.color) {
    sw.className = 'sw';
    sw.style.background = colour(h.color);
  } else if (h.note) {
    sw.className = 'sw';
    ...
    sw.style.color = colour('underline');
  }
  return sw;
}
...
async function drawRecent() {
  ...
  const custom = all[PALETTE_KEY] ?? {};

  // Tarama sonucu konsola: "bos" ile "okuyamadim" arasindaki farki gormek icin
  // popup u incelemek yeterli olsun, tahmin gerekmesin.
  const docKeys = Object.keys(all).filter((k) => k.startsWith(String.fromCharCode(100, 111, 99, 58)));
  console.log(
    `[DocHL popup] storage anahtari: ${Object.keys(all).length}, doc kaydi: ${docKeys.length}`,
    docKeys,
  );
  ...
  for (const { key, rec } of docs) {
    ...
    for (const h of marks.slice(0, MARKS_PER_DOC)) {
      ...
      li.append(swatchFor(h, custom), ex);
      list.appendChild(li);
    }
...
async function drawAccess() {
   ...
   * popup MAY CLOSE while it is up. The permission is still granted; reopening
   * the popup shows the correct state. That is Chrome behaviour, not a bug here.
   *
 ...
 * you that.
 *
 * The rows are built ONCE and updated in place. Rebuilding them on every input
 * event destroyed the <input type="color"> the native picker was attached to,
 * which slammed it shut the moment a colour was touched.
 * ------------------------------------------------------------------------- */
...
async function drawColours() {
  ...
  /** name -> { mark, ratio, input } so a change touches one row, not all of them. */
  const rows = new Map();
  ...
  async function applyColour(name, value) {
    ...
    paintRow(name);
    // Straight to storage: the content script listens and repaints every open
    // page, so the choice is judged on real text while the picker is still open.
  ...
  for (const name of Object.keys(DEFAULTS)) {
    ...
    if (name === 'underline') row.dataset.kind = 'rule';
    ...
    row.append(mark, ratio, input);
    strokeBox.appendChild(row);
...
async function drawAddons() {
  ...
  // The API lives on the page, not on chrome.*: a feature check, not a version one.
  if (typeof self.Translator?.create !== 'function') {
    badge.textContent = t('qmUnavailable');
    note.textContent = t('trNeedsChrome');
    return;
  }
  ...
  field.hidden = false;
  note.textContent = t('trModelNote');
 ...
 * wanting it gone.
 *
 * prefers-reduced-motion is honoured without this switch. The switch is for
 * everyone who never set that flag.
 * ------------------------------------------------------------------------- */

async function drawMotion() {
  const box = document.getElementById('pulse');
  const prefs = (await chrome.storage.local.get(PREFS_KEY))[PREFS_KEY] ?? {};
  box.checked = prefs.pulse !== false;
  ...
  box.addEventListener('change', async () => {
    ...
    await chrome.storage.local.set({ [PREFS_KEY]: { ...current, pulse: box.checked } });
  });
}
...
// Independent of each other, in the order a reader meets them.
panel('recent', 'recent', drawRecent);
panel('access', null, drawAccess);
panel('colours', 'strokes', drawColours);
panel('addons', null, drawAddons);
panel('motion', null, drawMotion);
panel('about', null, drawAbout);
