...
| Sürüm | Ne geldi |
|---|---|
| **0.16.0** | Eklenti **Notestark** adını aldı — `notestark.com` altındaki not uygulamasının tarayıcı tarafı. Store başlığı, promo görselleri ve gizlilik metni elle düzenlenmedi, tek kaynaktan yeniden üretildi. **Bilinçli olarak değişmeyenler:** storage key'leri (`doc:`, `hash:`, `dhPalette`, `dhPanelPrefs`, `dhTranslate`), CSS prefix'leri (`dh-`, `data-dh-`) ve diagnostics handle'ı. Bunları yeniden adlandırmak kayıtlı her highlight'ı orphan yapardı; karşılığında yalnızca hiçbir okuyucunun görmediği bir iç isim düzelirdi. Ürün adı ile iç isimlendirme ayrı şeyler, yalnızca biri değişti |
| 0.16.0 | **Recent sekmesi** — menü açıldığında en son işaretlediklerin, işaretlediğin yere göre gruplu: web sitesi ya da lokal doküman için dosya adı. Her grup katlanabiliyor ve o yerdeki her şey tek tuşla (iki adımlı onayla) silinebiliyor. Yeni bir şey saklanmıyor; kayıtlar zaten doküman bazında tutulduğu için bu onları okuyor — ikinci bir liste yalnızca senkronu bozabilecek bir şey olurdu. Silme, kaydın yanında hash index'ini de kaldırıyor: geride kalsa, adı değişmiş bir dosya artık var olmayan bir dokümanı diriltebilirdi |
| 0.16.0 | **Silgi seçime saygı duyuyor** — önceden yalnızca tüm dokümanı silebiliyordu, yani basmaya cesaret gerektiren bir düğmeydi. Artık metin seçiliyken sadece seçimi siliyor; iki adımlı onay, gerçekten hak ettiği yerde — her şeyi alan tarafta — duruyor. Tooltip hangisinin kurulu olduğunu yazıyor |
| 0.16.0 | **Notun kaynağı veride tutuluyor ve gösteriliyor** — çeviriden gelen not mavi daire, kendi yazdığın amber köşe. Renk tek başına zayıf bir etiket olurdu (iki koyu soğuk ton, renk körlüğünün tam da düzlediği çift), o yüzden şekil de taşıyor. Kendi yazdığın notta alan hiç bulunmuyor: mevcut her not migration'sız doğru tarafa düşüyor |
| 0.16.0 | **Nokta ait olduğu işaretin rengini alıyor** ve ink'e doğru karıştırılıyor — sarı bir metnin üstündeki işaretçi sarıdan koyu olmak zorunda. Karışım ölçüldü: ilk denemem 2.70:1 ile WCAG 1.4.11'in (non-text UI için 3:1) altında kalmıştı, 0.70 katsayısıyla 3.86:1'e çıkıyor. Panel de sayfayla aynı üç durumu çiziyor artık — eksik bir dal yüzünden renksiz bir işaret underline chip'ine düşüyor, çeviri notu panelde kırmızı sayfada mavi görünüyordu |
| 0.16.0 | **Pulse kapatılabiliyor.** Nöbet eşiği saniyede üç flaş, buradaki 0.67 Hz — çok altında. Asıl aşılan kriter **WCAG 2.2.2** (Pause Stop Hide): beş saniyeden uzun hareket durdurulabilmeli, üç döngü + stagger 5.4 saniyeye çıkmıştı. Stagger 400 ms'e sınırlandı (4.9 sn) **ve** ayara bağlandı. Varsayılan açık; `prefers-reduced-motion` kimse ayarı bulmadan zaten kazanıyor. Testlerin üçü eşikleri **kaynaktan okuyup** kontrol ediyor, yani sınır yorum satırı değil |
| 0.16.0 | Panel sayfaya yer açarken artık **ölçüyor**: `html`'e verilen margin, kök elemanın genişliği auto değilse tarayıcı tarafından sessizce düşürülüyor (over-constrained) — hata da uyarı da yok, panel metnin üstüne biniyor. Kural yazılıp geri okunuyor, kök reddettiyse `body`'ye geçiliyor. MDN'de ölçüldü: `html` 0px, `body` 332px |
| 0.15.0 | **Pasajı cihazında çevir** — Chrome'un yerleşik Translator'ı. Bu özelliğin var olma sebebi tam olarak bu: pasajı bir çeviri servisine göndermek eklentinin geri kalanının üzerine kurulduğu sözü bozardı ve hem gizlilik metniyle hem store'a beyan edilenle çelişirdi. Seçim toolbar'ındaki dil düğmesi hedefi gösterip tıklayınca çeviriyor, yanındaki caret kısa bir hedef listesi açıyor; tam liste popup'ta. Çeviri **not olarak saklanabiliyor** — saklamak istediğin bir çeviri zaten o pasaj hakkında bir nottur. Projede hiçbir yerde dil adı listesi yok: kod saklanıyor, `Intl.DisplayNames` okuyucunun kendi dilinde yazıyor, yani otuz dil hiç çeviri işi doğurmadı. Masaüstünde Chrome 138+ gerekiyor |
| 0.14.0 | **Sağ tık menüsü** — toolbar ve panelin yaptığını fareyi pasajdan kaldırmadan sunuyor. Menü service worker'da yaşıyor, ne seçimi ne sayfayı görebiliyor; her madde content script'e gönderilen tek bir komutla yürütülüyor. Seçime bağlı maddeler selection-only işaretli, menü `file://` ve http(s) ile sınırlı — cevap verecek bir şeyin olmadığı yerde hiç çıkmıyor. `contextMenus` izni eklendi, yani manifest ve store izin listesi değişti. **Testler gerçek bir kusur buldu:** not yazma önce mark'ı tek boşluk placeholder ile oluşturup sonra dolduruyordu; boşluk hiç hayatta kalmıyordu (`applyToSelection` notu trim ediyor, boş olan `undefined` oluyor), yani not komutu hiçbir şey üretmiyordu. Dahası vazgeçmek geride renksiz, altı çizilisiz, notsuz — hiçbir şey boyamayan görünmez bir kayıt bırakıyordu |
| 0.13.1 | **Renk seçici ilk tıklamada açılıyor.** Satır bir `<button>`, renk `<input>`'u onun içindeydi — button içinde interactive content geçersiz HTML ve button tıklamayı yutuyordu. Tarayıcıda üç aday yapı yan yana ölçüldü: input tıklamayı **hiç almıyordu**. Yedi sekiz denemenin sebebi buydu; her biri satırın ilettiği bir tıklama değil, input'un kendi kutusuna ıskalayan bir tıklamaydı. `<label>` sarmaladığı input'a tıklamayı **native olarak** iletiyor |
| 0.13.0 | **Menü ürünün kendi diliyle yeniden tasarlandı.** Öncesi bir kopyaydı: sol ikon rayı, jenerik ikonlar, model alınan eklentiyle aynı şekil. Artık bu ürünün ne olduğundan kuruluyor — bir fosforlu kalem. Aktif sekme pill ya da ray maddesi değil, hafif eğri çizilmiş, köşeleri düzensiz, kelimeyi gerçek bir kalem gibi taşan **sarı bir çizgi**. Renkler kare olarak değil, **kendi adlarının üstüne çekilmiş çizgi** olarak gösteriliyor: bir fosforlu kalem, metnin onun *içinden* nasıl göründüğüyle değerlendirilir ve bir kare bunu gösteremez. Underline ne ise o çiziliyor — metnin altında bir çizgi, arkasında bir yıkama değil |
| 0.12.0 | **Hızlı menü ve değiştirilebilir palet.** Popup tek sütun düğmeydi; ikon rayının arkasında dört sekme oldu. Palet değiştirilebiliyor: varsayılanlar göz kararı değil ölçülerek seçildiği için picker, seçerken ink'e karşı kontrastı gösteriyor ve WCAG AAA altında kalanı işaretliyor — özel renk kullanıcının kararı, ama "highlight'larım okunmuyor" sonradan keşfedilip eklentiye yazılmamalı. Değişiklik storage'a düşüyor ve açık her sayfa reload'suz yeniden boyanıyor |
| 0.11.0 | **Markdown preview ve Notes sekmesi.** Lokal bir `.md` dosyası otomatik render ediliyor — başlık, kod, liste, tablo, alıntı — ve kaynağa dönüş tek düğme. Motorun dokümanı bilerek yeniden yazdığı tek yer burası ve bu bir çelişki değil: "DOM'a dokunma" kuralı **kazara** mutasyonun anchor'ları bozmasını engellemek için var, görünüm değiştirmek kazara değil. Bedeli de karşılanmış durumda: anchor'lar düzleştirilmiş metne quote + offset, render metni değiştiriyor (`## Başlık` → `Başlık`), markdown sözdizimini kapsayan bir işaret render görünümünde çözülmüyor — ama kaybolmuyor, orphan olarak saklanıp görünüm kapatılınca geri geliyor. Renderer bize ait: remote code'a ne CSP ne store'a beyan edilen izin veriyor. `.md` untrusted input olduğu için önce HTML escape ediliyor ve link şemaları filtreleniyor (script tag, inline handler ve `javascript:` URL ile test edildi) |
| 0.10.2 | Not noktası düzleştirildi — beyaz halka ve gölge kaldırıldı. Koyu bir sayfada halka, işaretin parçası gibi değil metne yapıştırılmış bir çerçeve gibi okunuyordu. Nokta artık düz renk, biraz daha küçük, ve açık durumunu outline ile değil koyulaşıp büyüyerek gösteriyor. Korunan tek halka `:focus-visible` — halkayı göstermenin **amacın kendisi** olduğu yer |
